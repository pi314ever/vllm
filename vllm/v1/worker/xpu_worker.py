...
class XPUWorker(Worker):
    ...
    def sleep(self, level: int = 1) -> None:
        ...
        if hasattr(self.model_runner, "kv_caches"):
            ...
            self._saved_kv_cache_config = getattr(self.model_runner,
                                                  "kv_cache_config", None)
            # Clear KV cache tensors
            if hasattr(self.model_runner, "kv_caches"):
                del self.model_runner.kv_caches
                for (layer_name
                     ) in self.compilation_config.static_forward_context:
        ...
        logger.warning(
            ...
            free_bytes_after_sleep / GiB_bytes,
            total_bytes / GiB_bytes,
        )
    ...
    def wake_up(self, tags: Optional[list[str]] = None) -> None:
        ...
        if "kv_cache" in tags:
            ...
            if self._saved_kv_cache_config is not None:
                self.model_runner.initialize_kv_cache_tensors(
                    self._saved_kv_cache_config)
