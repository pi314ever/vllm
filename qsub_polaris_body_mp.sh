...
if [[ "${RUN_SERVING}" == "1" ]]; then
	...
	# Pre-flight: refuse to start if ${SERVE_PORT} is already in LISTEN
	# state on the head node. vLLM sets SO_REUSEPORT on the API-server
	# socket (vllm/entrypoints/openai/api_server.py:503), so a stale
	# listener (e.g. from a prior failed run on the same head node)
	# would silently share the port and answer benchmark traffic on our
	# behalf, while our new server stays bound-but-not-listening for
	# the entire ~15 min model load. There is no readiness probe that
	# can recover from this once it has happened, so we fail fast here
	# instead.
	...
	#     AsyncLLM.check_health (vllm/v1/engine/async_llm.py:866-869)
	#     and only becomes reachable once uvicorn starts serving in
	#     build_and_serve() (vllm/entrypoints/openai/api_server.py:604,
	#     vllm/entrypoints/launcher.py:82), which runs only after
	#     engine init + KV cache alloc + warmup are complete on every
	#     worker (vllm/v1/engine/core.py:116, 126, 1411-1416). curl -f
	#     rejects 5xx, so a 503 from EngineDeadError is treated as
	#     not-ready.
	...
	#     vllm/benchmarks/lib/ready_checker.py:18-79 uses internally
	#     when --ready-check-timeout-sec > 0.
	#
	# MAX_WAIT=1800: DeepSeek-V3 cold-start on Polaris (lustre
	# first-touch) observed at ~893s (model load + init engine +
	# warmup); warm-cache runs land closer to ~5 min. 30 min ceiling
	# leaves ~25 min of the 1h walltime for the sweep + cleanup.
